...
export const PostIdForm = ({ postId }: PostIdFormProps) => {
  const post = postMockData.find((e) => e.post_id === Number.parseInt(postId))
  ...
  return <form className="flex flex-col h-[300px] mt-12 border-b border-[#D9D9D9]">
    ...
    <div className="flex flex-start gap-2 items-center pt-2">
      <img src={post.author_image} className="w-9 h-9" alt="author_image" />
    ...
    <div className="relative p-4 flex flex-grow border-[#D9D9D9] border-b pb-6 justify-between">
      <div className="flex gap-4 items-center">
        ...
        <img onClick={handleComment}
          className="cursor-pointer" src="/images/comment.svg" alt="comment" />
        <img
          onClick={handleShare}
          className="cursor-pointer" src="/images/share.svg" alt="share" />
      </div>
      <div>
        <img
          className="cursor-pointer" src="/images/bookmark.svg" alt="bookmark" />
      </div>
